<?php
	...
	class LdapAccess implements DatabaseAccess{
		...
		/*
		 * Diese Funktion prüft, ob ein Benutzer ein System-Administrator ist
		 * 
		 * $uid:		Benutzername der zu überprüfenden Person
		 * 
		 * return:		true/false
		 */
				...
				if($size === 1){
					...
					*/
					if($groupname != '' AND $groupname != $currentGroup){
						...
						//prüfen, ob die gruppe noch untergruppen besitzt
					...
					// Gruppenverlinkungen in den Benutzereinträgen aktualisieren
			...
			if($connection != false){
				...
				if($entries['count'] === 1){
					...
					if($group_name_new != '' AND $group_name_new != $group_name){
						...
						// prüfe, ob gruppe noch untergruppen besitzt
						...
						elseif($entries_subgroups['count'] > 1){
							...
							for($i=0; $i<$entries_subgroups['count']; $i++){
								...
								// daten für den zu kopierenden eintrag ermitteln
								$data = array();
								$data['objectClass'][0] = 'groupOfURLs';
								$data['objectClass'][1] = 'top';
				...
				if($parent_new != ''){
					...
					// dn der gruppe nochmals ermitteln, da er sich ggf. durch umbenennen geändert hat
		...
		public function moveUser($uid, $target){
			...
			if($connection != false){
				...
				if($target == 'TRASH'){ 
					...
					// Einträge aus dem Gruppeneintrag entfernen
					/*
				...
				if($target == 'POOL'){
					...
					//Einträge aus dem Benutzereintrag entfernen
...
*/
				...
				if($target != 'TRASH' AND $target != 'POOL'){
					...
					if(strpos($userDN, 'ou=TRASH') != false){
						...
						// die member-Attribute in den Gruppeneinträgen wiederherstellen
						$result = ldap_search($connection, $userDN, 'uid='.$uid, array($memberAttr));
						$entries = ldap_get_entries($connection, $result);
						if($entries['count'] === 1){
							foreach($entries[0][strtolower($memberAttr)] AS $groupDN){
